...
function addRowtoTable() {
    ...
    // Update the data-id attribute to make it unique
    let newRowIds = new Date().getTime(); // Use current timestamp as a unique identifier
    newRow.setAttribute('data-id', newRowIds);

    // Update names and IDs for the new row
    newRow.querySelectorAll('[id]').forEach(function(element) {
        element.id = element.id.replace(/-\d+-/, `-${newRowIds}-`);
    });

    newRow.querySelectorAll('[name]').forEach(function(element) {
        element.name = element.name.replace(/-\d+-/, `-${newRowIds}-`);
    });
